const {userModel} = require('../models')
const bcrypt = require('bcrypt')
const jwtUtils = require('../utils/jwt.utils')
const { where } = require('sequelize')


module.exports = {
    getAll : function(req,res){
        userModel.findAll()
        .then((data) =>{
            res.status(200).json(
                {
                    "status": "success",
                    "data": data
                }
            )
        })
        
    },
    getById:function(req,res) {                                                                                                         
        let id = req.params.id;
        userModel.findByPk(id)
           .then((data)=>{
               if (!data) {
                   res.status(404).json({
                       'status': 'error',
                       'message': 'user '+ data +'invalide'
                   });
               }
                res.status(200).json({
                    'status': 'succes',
                    'data': data
                });
        }).catch(err=>{
            console.log('Erreur : ', err.message)
        })
            
        
    },
    add: function (req, res) { 
        let {nom, email, role,mdp}= req.body;

        if(nom==null || email==null || role==null || mdp==null){
            res.status(409).json({
                'status': 'error',
                'message': 'Donnees incompletes'
            });
            return;
        }
        bcrypt.hash(mdp,5,(err,encrypted)=>{
            userModel.create({
                nom: nom,
                email: email,
                role: role,
                mdp: encrypted
            }).then((data) => {
                res.status(201).json({
                    'status': 'success',
                    'message': 'user ajoute' + data.nom ,
                    'data': data
                });
            }).catch(err => {
                console.log('Erreur :', err.message)
            })
        })
        
        
        
    },
    update: function (req, res) { 
        let id = req.params.id;

        let { nom, email, role, mdp } = req.body;
        if (nom == null || email == null || role == null) {
            res.status(409).json({
                'status': 'error',
                'message': 'Donnees incompletes'
            });
            return;
        }

        // Si le mot de passe est fourni et non vide, on le met à jour
        if (mdp && mdp.trim() !== '') {
            bcrypt.hash(mdp, 5, (err, encrypted) => {
                userModel.update(
                    { nom: nom, email: email, role: role, mdp: encrypted }, 
                    { where: { id: id } }
                ).then((data) => {
                    if (!data[0]) {
                        res.status(404).json({
                            'status': 'error',
                            'message': 'user id invalide'
                        });
                        return;
                    }

                    res.status(200).json({
                        'status': 'success',
                        'message': 'user modifie'
                    });
                }).catch(err => {
                    console.log('Erreur:', err.message);
                    res.status(500).json({
                        'status': 'error',
                        'message': 'Erreur lors de la mise à jour'
                    });
                });
            });
        } else {
            // Mettre à jour sans changer le mot de passe
            userModel.update(
                { nom: nom, email: email, role: role }, 
                { where: { id: id } }
            ).then((data) => {
                if (!data[0]) {
                    res.status(404).json({
                        'status': 'error',
                        'message': 'user id invalide'
                    });
                    return;
                }

                res.status(200).json({
                    'status': 'success',
                    'message': 'user modifie'
                });
            }).catch(err => {
                console.log('Erreur:', err.message);
                res.status(500).json({
                    'status': 'error',
                    'message': 'Erreur lors de la mise à jour'
                });
            });
        }
    },
    delete: function (req, res) { 
        let id = req.params.id;
        userModel.destroy(
            {where :{id:id}}
        ).then((data)=>{
            if (!data) {
                res.status(404).json({
                    'status': 'error',
                    'message': 'user id invalide'
                });
                return;
            }
            res.status(204).json({
                'status': 'success',
                'message': 'user supprime'
            });
        }).catch(err=>{
            console.log('Erreur :', err.message)
        })
       
    },
    login : function(req,res) {
        let {email,mdp} = req.body;
        if(email==null || mdp==null){
            res.status(400).json({
                'status' : 'error',
                'message': 'Donnees incompletes pour authentification'
            });
            return;
        }
        userModel.findOne({where : {email:email}})
        .then((userFound) =>{
            if(userFound){
                if(userFound.role === 'admin'){
                    bcrypt.compare(mdp, userFound.mdp, (err, resBcrypt) => {
                        if (resBcrypt) {
                            res.status(200).json({
                                'status': 'success',
                                'userId': userFound.id,
                                'token': jwtUtils.generateTokenForUser(userFound)
                            })
                            return;
                        } else {
                            res.status(403).json({
                                status: 'error',
                                message: 'Donnees de connexion invalides'
                            })
                        }
                    })
                }else{
                    res.status(403).json({
                        status: 'error',
                        message: 'Utilisateur non autorise'
                    })
                }
                
            }else{
                res.status(403).json({
                    status: 'error',
                    message: 'Donnees de connexion invalides'
                })
            }
        })
    }
}